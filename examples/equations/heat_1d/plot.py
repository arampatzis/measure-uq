"""
Plot the solution of the 1D heat equation.

This module provides functionality to visualize solutions of the 1D heat equation,
...
def plot(
    ...
    t = tensor(np.linspace(0, T, 50)[:, None])
    x = tensor(np.linspace(0, X, 100)[:, None])

    parameters = pde.parameters_train.values.detach().cpu()
