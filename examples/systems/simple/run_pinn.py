#!/usr/bin/env python3
r"""
Solution of a simple system of ODEs.
...
from examples.systems.simple.ode import (
    InitialCondition,
    RandomParameters,
    Residual,
    analytical_solution,
...
def train() -> None:
    ...
    conditions = [
        Residual(N=3000, T=2),
        InitialCondition(initial_values=torch.tensor([[30, 4]])),
    ]
    ...
    stoppers = Stoppers(
        trainer_data=trainer_data,
        stoppers=[
            TrainingLossStopper(patience=50, delta=1e-4),
        ],
    )
    ...
    pde.save("data/simple_pde.pickle")
    model.save("data/simple_model.pickle")
    trainer.save("data/simple_trainer.pickle")


def plot_loss() -> None:
    """Plot the solution of the ODE."""
    trainer = Trainer.load("data/simple_trainer.pickle")
