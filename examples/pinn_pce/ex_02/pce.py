...
# ax2_line, = axs[2].plot(x, np.abs(np.mean(solutions[:, :, k], axis=0) - mean_solution[k]))
# ax3_line, = axs[3].plot(x, np.abs(np.std(solutions[:, :, k], axis=0) - std_solution[k]))
...
def animate(i):
    ...
    # e = np.abs(np.mean(solutions[:, :, i], axis=0) - mean_solution[i])
    # ax2_line.set_ydata(e)
    # axs[2].set_ylim(min(e), max(e))
    
    # e = np.abs(np.std( solutions[:, :, i], axis=0) - std_solution[i] )
    # ax3_line.set_ydata(e)
    # axs[3].set_ylim(min(e), max(e))
     
    # return ax0_lines, ax1_mean, ax1_fill, ax2_line, ax3_line,
    ...
    return ax0_lines, ax1_mean, ax1_fill, ax2_line1, ax2_line2, ax3_line1, ax3_line2,
